...
Emacs만 종료했다면 `eam-attach`로 살아 있는 세션을 연결한다. CLI 자체가 종료됐으면 `eam-resume`에서 저장 대화를 선택해 새 CLI로 재개한다. Python 구현에서 실행한 기존 세션은 자동 변환하지 않으며 native 사용을 위해 정상 종료 후 재개한다.
