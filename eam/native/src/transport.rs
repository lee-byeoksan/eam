...
fn spawn(v: &Value, mut s: libc::winsize) -> Result<(Child, File)> {
    ...
    c.current_dir(string(v, "directory")?)
    ...
    let editor = format!(
        "{} editor {}",
        shquote(&std::env::current_exe()?.to_string_lossy()),
        shquote(string(&launch, "editor")?)
    );
...
pub fn run(path: &Path) -> Result<i32> {
    ...
    let result = (|| -> Result<()> {
        while !STOP.load(Ordering::Relaxed) {
            ...
            let source = dest;
            let mut fds = [
                pfd(master.as_raw_fd(), !eof, !input.is_empty() && !eof),
                pfd(
                    source,
            ...
            if readable(&fds[4]) {
                ...
                if let Ok((n, addr)) = sock.recv_from(&mut b) {
                    if let Ok(q) = serde_json::from_slice::<Value>(&b[..n]) {
                        if q["id"] == v["id"]
                            && ["status", "stop"].contains(&q["action"].as_str().unwrap_or(""))
                        {
                            let reply = json!({"id":v["id"],"state":"running","recorder_pid":child.id(),"attached_clients":usize::from(client.is_some()),"output_bytes":total,"replay_bytes":replay.len(),"last_input_at":last_input,"last_output_at":last_output});
            ...
            if readable(&fds[0]) {
                ...
                if n == 0 {
                    ...
                    if client.is_some() {
                        if push(&mut output, &packet(b'O', b)).is_err() {
                            client = None;
                            output.clear();
                            replay_send.clear();
                            incoming.clear();
            ...
            if fds[2].revents & libc::POLLOUT != 0 && dest >= 0 {
