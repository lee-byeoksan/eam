...
(defun eam-history ()
  "Choose native saved conversation history for this CLI's project."
  (interactive)
  (require 'eam-native-history)
  (unless eam-terminal--current (user-error "Use a CLI or draft buffer"))
  (eam-native-history-open (eam-terminal-name eam-terminal--current)
                           (eam-terminal-directory eam-terminal--current)))
(defun eam-paste ()
...
(defun eam-terminal-start (name executable args directory)
  "Start a persistent interactive CLI in a PTY, with a separate draft."
  (make-directory eam-directory t)
  (let* ((file (and eam-record-terminal (make-temp-file (expand-file-name "terminal-" eam-directory) nil ".ansi")))
         (project-directory (file-name-as-directory (expand-file-name directory)))
         (s (eam-terminal--session
    ...
    (unwind-protect
      ...
      (unless started
        ...
        (dolist (buffer (list (eam-terminal-input s) (eam-terminal-output s)))
...
(defun eam-terminal--read-project-directory ()
  "Prefill the caller's current directory even when defaults are hidden."
